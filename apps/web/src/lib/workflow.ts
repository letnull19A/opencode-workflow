...
import type { WireEvent, ModuleAction, ModuleDomain, PipelinePhase, WireWorkflow } from "./types";
import { ACTION_PHASES } from "./types";
import { listWorkflows, startWorkflow, stopWorkflow } from "./api";
    ...
    if (event.type === "pipeline.started") {
      ...
      if (!existing) {
        ...
        runs[runId] = {
          ...
          title: event.task.title,
          source: event.task.source,
          workflow: existing.workflow ?? event.workflow,
  ...
  switch (action.type) {
    ...
    case "registered": {
      ...
      if (!runs[action.runId]) {
        runs = { ...runs, [action.runId]: makeRun(action.runId, { title: action.title, workflow: action.workflow }) };
        ...
        runs = {
          ...
          [action.runId]: {
            ...run,
            title: action.title,
            workflow: run.workflow ?? action.workflow,
            updatedAt: Date.now(),
          },
        };
      }
      return pushLog(
        { ...state, runs, order },
        {
          ts: action.ts,
          severity: "accent",
          text: `started "${action.title}" → ${action.runId}${action.workflow ? ` [${action.workflow}]` : ""}`,
        }
      );
    }
...
if (event.type === "pipeline.started") {
        ...
        const runs = {
          ...
          [event.runId]: run
            ? {
                ...
                title: event.task.title,
                source: event.task.source,
                workflow: run.workflow ?? event.workflow,
                updatedAt: action.ts,
              }
            : makeRun(event.runId, {
                title: event.task.title,
      ...
      if (event.type === "workflow.registered" || event.type === "workflow.updated") {
        const kind = event.type === "workflow.registered" ? "workflow registered" : "workflow updated";
        return pushLog(
          state,
          {
            ts: action.ts,
            severity: "accent",
            text: `${kind} → ${event.workflowId} (${event.label})`,
          }
        );
      }

      if (event.type === "workflow.removed") {
        return pushLog(state, {
          ts: action.ts,
          severity: "muted",
          text: `workflow removed → ${event.workflowId}`,
        });
      }
      ...
      if (event.type === "workflow.error") {
        return pushLog(state, {
          ts: action.ts,
          severity: "error",
          text: `workflow errored → ${event.workflowId}: ${event.error}`,
        });
      }
      return state;
    }
...
export function useWorkflow() {
  ...
  const [workflows, setWorkflows] = useState<WireWorkflow[]>([]);
  ...
  const reloadWorkflows = useCallback(async () => {
    try {
      ...
      dispatch({
        type: "note",
        text: `list workflows: ${err instanceof Error ? err.message : String(err)}`,
        severity: "error",
        ts: Date.now(),
      });
    }
  ...
  useEffect(() => {
    ...
    es.addEventListener("workflow.registered", wire("workflow.registered"));
    es.addEventListener("workflow.updated", wire("workflow.updated"));
    es.addEventListener("workflow.removed", wire("workflow.removed"));
    es.addEventListener("workflow.error", wire("workflow.error"));
  ...
  const registerAndStart = useCallback(
    async (workflowId: string, title: string, meta?: Record<string, unknown>) => {
      const titleClean = title.trim();
      if (!titleClean) throw new Error("title is required");
      const res = await startWorkflow(workflowId, titleClean, meta);
      dispatch({
        type: "registered",
        runId: res.runId,
        title: titleClean,
  ...
  const stopRun = useCallback(async (workflowId: string, runId: string) => {
    try {
      await stopWorkflow(workflowId, runId);
      dispatch({
        type: "note",
  ...
  return { state, registerAndStart, stopRun, workflows, reloadWorkflows };
}
