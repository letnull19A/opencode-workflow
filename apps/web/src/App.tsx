...
export default function App() {
  const { state, registerAndStart, stopRun, workflows, reloadWorkflows } = useWorkflow();
  ...
  const stopSelected = async () => {
    ...
    setStopping(true);
    try {
      await stopRun(selectedRun.workflow ?? "module", selectedRun.runId);
    } catch {
  ...
  return (
    <div className="flex h-screen flex-col bg-background text-foreground">
      <header className="flex items-center gap-4 border-b px-4 py-2.5">
        ...
        <span className="text-[11px] text-muted-foreground">workflow platform dashboard</span>
      ...
      <div className="flex min-h-0 flex-1">
        <aside className="w-80 shrink-0 overflow-y-auto border-r p-3">
          <Sidebar
            ...
            onStop={stopRun}
            onWorkflowsRefresh={() => void reloadWorkflows()}
          />
        </aside>
